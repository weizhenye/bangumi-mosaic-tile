...
function getWikisUntil(username, type, page, lastUpdated) {
  return getWikis(username, type, page).then((wikis) => (
    !wikis.length || wikis.find((date) => new Date(`${date}T00:00:00.000+08:00`) <= lastUpdated)
      ? wikis.filter((date) => new Date(`${date}T00:00:00.000+08:00`) > lastUpdated)
...
module.exports = function main(username, type, lastUpdated) {
  return checkUsername(username)
    .then(() => getWikisUntil(username, type, 1, lastUpdated - 8.64e7 * 7))
    .then((wikis) => {
      const data = {};
