...
function getTimelinesUntil(username, type, page, lastUpdated) {
  return getTimelines(username, type, page).then(timelines => (
    !timelines.length || timelines.find(({ date }) => new Date(date) - 2.88e7 <= lastUpdated)
      ? timelines.filter(({ date }) => new Date(date) - 2.88e7 > lastUpdated)
...
module.exports = function main(username, type, lastUpdated) {
  return checkUsername(username)
    .then(() => getTimelinesUntil(username, type, 1, lastUpdated))
    .then(timelines => (
